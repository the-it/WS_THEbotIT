...
class TestTemplateFinder(TestCase):
    ...
    def test_find_two_templates(self):
        ...
        self.assertListEqual(
            [TemplatePosition(0, 12, "{{Template}}"), TemplatePosition(12, 24, "{{Template}}")],
            finder.get_positions("Template"),
        )
    ...
    def test_find_nested_template(self):
        ...
        self.assertListEqual(
            [TemplatePosition(0, 30, "{{Template|{{OtherTemplate}}}}")], finder.get_positions("Template")
        )

    def test_find_nested_template_with_offset(self):
        finder = TemplateFinder("1234567890{{Template|{{OtherTemplate}}}}")
        self.assertListEqual(
            [TemplatePosition(10, 40, "{{Template|{{OtherTemplate}}}}")], finder.get_positions("Template")
        )

    def test_find_complex(self):
        finder = TemplateFinder("{{Template|{{{}}{{}}}}")
        self.assertListEqual([TemplatePosition(0, 22, "{{Template|{{{}}{{}}}}")], finder.get_positions("Template"))
