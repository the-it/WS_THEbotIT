...
class TestERROTask(TestCase):
    ...
    def test_process(self):
        with LogCapture():
            ...
            task.append_error(":RE:Lemma1", "service.ws_re.template.ReDatenException: "
                                            "The count of start templates doesn't match the count of end templates.")
            task.append_error(":RE:Lemma2", "service.ws_re.template.ReDatenException: "
            ...
            self.assertRegex(task._build_entry(), "\n\n==\\d{4}-\\d{2}-\\d{2}==\n\n"
                                                  ...
                                                  "\\{'key': 'GEMEINFREI', 'value': '2024'\\}"
                             )

    def test_finish_up(self):
        with mock.patch("service.ws_re.scanner.tasks.base_task.pywikibot.Page") as page_mock:
            with mock.patch("service.ws_re.scanner.tasks.base_task.pywikibot.Page.text",
                            new_callable=mock.PropertyMock(return_value="bla")) as text_mock:
