...
class Lemma():
    ...
    def get_link(self) -> str:
        ...
        if redirect:
            link = f"[[RE:{self['lemma']}|''{{{{Anker2|{self._escape_link_for_templates(self['lemma'])}}}}}'']]"
    ...
    def _process_wiki_link(self, wiki_type: Literal["wp", "ws"]) -> Tuple[str, str]:
        ...
        return f"[[{self[link_type]}|{self._escape_link_for_templates(parts[-1])}<sup>({wiki_type.upper()} {parts[1]})</sup>]]", \
