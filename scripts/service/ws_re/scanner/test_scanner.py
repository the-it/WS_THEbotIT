...
from scripts.service.ws_re.scanner.scanner import ReScanner
...
class TestReScanner(TestCase):
    def setUp(self):
        self.petscan_patcher = mock.patch("scripts.service.ws_re.scanner.scanner.PetScan")
    ...
    def _mock_surroundings(self):
        ...
        lemma_patcher = mock.patch("scripts.service.ws_re.scanner.scanner.ReScanner.compile_lemma_list",
                                   mock.Mock())
        page_patcher = mock.patch("scripts.service.ws_re.scanner.scanner.pywikibot.Page")
        page_patcher_error = mock.patch("scripts.service.ws_re.scanner.tasks.base_task.pywikibot.Page")
        re_page_patcher = mock.patch("scripts.service.ws_re.scanner.scanner.RePage")
    ...
    def _mock_task(self):
        # pylint: disable=attribute-defined-outside-init
        task_patcher = mock.patch("scripts.service.ws_re.scanner.scanner.ReScannerTask.run")
    ...
    def test_watchdog(self):
        self._mock_surroundings()
        self.lemma_mock.return_value = [":RE:Lemma1", ":RE:Lemma2"]
        with mock.patch("scripts.service.ws_re.scanner.scanner.ReScanner._watchdog", mock.Mock(return_value=True)):
            with LogCapture() as log_catcher:
                with ReScanner(log_to_screen=False, log_to_wiki=False, debug=False) as bot:
                    log_catcher.clear()
                    bot.tasks = [self.ONE1Task]
